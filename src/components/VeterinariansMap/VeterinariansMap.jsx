...
const VeterinariansMap = ({ veterinarians = [] }) => {
  const markers = veterinarians
    .filter((v) => {
      const p = v.profile;
    ...
    .map((v) => {
      ...
      return {
        ...
        specialization: rsd.specialization || "",
        location: locationParts.length ? locationParts.join(", ") : (p.country || p.region || "—"),
        isVerified: v.isVerified === true || v.is_verified === true,
        position: [lat, lng],
      };
    })
    .filter(Boolean);
      ...
      {markers.length === 0 ? (
        ...
        >
          <Typography sx={{ color: "#000000", fontFamily: '"Calibri Light", Calibri, sans-serif' }}>
            No veterinarian locations with coordinates to show on the map.
          </Typography>
        </Box>
      ) : (
        <MapContainer
