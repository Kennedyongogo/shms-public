...
const SuppliersMap = ({ suppliers = [] }) => {
  const markers = suppliers
    ...
    .map((s) => {
      ...
      const productsSupplied = rsd.productsSupplied ? String(rsd.productsSupplied).trim() : "";
      const coverageArea = rsd.coverageArea ? String(rsd.coverageArea).trim() : "";
      return {
        id: s.id,
        fullName: s.fullName,
        ...
        productsSupplied,
        coverageArea,
        location: locationParts.length ? locationParts.join(", ") : (p.country || p.region || coverageArea || "—"),
        isVerified: s.isVerified === true || s.is_verified === true,
        position: [lat, lng],
      };
    })
    .filter(Boolean);
      ...
      {markers.length === 0 ? (
        ...
        >
          <Typography sx={{ color: "#000000", fontFamily: '"Calibri Light", Calibri, sans-serif' }}>
            No supplier locations with coordinates to show on the map.
          </Typography>
        </Box>
      ) : (
        <MapContainer
            ...
            <Marker key={marker.id} position={marker.position} icon={createGreenMarker()}>
              <Popup>
                <Box sx={{ minWidth: "200px", maxWidth: "320px", fontFamily: '"Calibri Light", Calibri, sans-serif', color: "#000000" }}>
                  ...
                  {marker.productsSupplied && (
                    <Typography variant="body2" sx={{ color: "#000000", fontFamily: '"Calibri Light", Calibri, sans-serif', fontSize: "0.875rem", mb: 0.5 }}>
                      {marker.productsSupplied}
                    </Typography>
                  )}
                  {marker.isVerified && (
